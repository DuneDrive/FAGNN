...
#root_folder = '/home/alex/hsm/GNN/BrainGNN_Pytorch/data/'
#data_folder = os.path.join(root_folder, 'APOE2/full_connect/sex_vertex_32')


phenotype = os.path.join('/mnt/paros_WORK/hsm/FAGNN/BC_GNN_hsm/APOE234/data/', 'APOE234_mrtrix/APOE234_Phenotype_mrtrix.csv')
behavior_data_path = '/mnt/paros_WORK/hsm/FAGNN/BC_GNN_hsm/APOE234/data/APOE234_mrtrix/behavioral_data_norm/'
...
def subject_connectivity(timeseries, subjects, atlas_name, kind, iter_no='', seed=1234,
                         n_subjects='', save=True, save_path=''):
    """
...
def get_ids(root_folder,data_folder):
    """
    return:
        subject_IDs    : list of all subject IDs
    """

    subject_IDs = np.genfromtxt(os.path.join('/mnt/paros_WORK/hsm/FAGNN/BC_GNN_hsm/APOE234/data/APOE234_mrtrix/', 'subject_IDs.txt'), dtype=str)
...
def get_subject_behavior(subject_list, score):
    scores_dict = {}
    behavior_data = {}
    ...
    with open(phenotype) as csv_file:
        ...
        for row in reader:
           ...
           #print(scores_dict[row['SUB_ID']])
            
            
            data_path = behavior_data_path + 'behavioral_' + scores_dict[row['SUB_ID']] + '.csv'
            my_data_raw = genfromtxt(data_path, delimiter=',')
            my_data = my_data_raw[1:,:]
            #print(my_data)
            behavior_data[row['SUB_ID']] = my_data
...
def get_networks(subject_list, kind, iter_no='', seed=1234, n_subjects='', atlas_name="aal",
                 variable='connectivity',data_folder='data'):
    """
