...
#from torch_geometric_data_hsm import Data
...
def split(data, batch):
    ...
    if data.y is not None:
        if data.y.size(0) == batch.size(0):
            ...
            slices['y'] = torch.arange(0, batch[-1] + 2, dtype=torch.float)
    if data.pos is not None:
        slices['pos'] = node_slice
    if data.pheno is not None:
        if data.pheno.size(0) == batch.size(0):
            slices['pheno'] = node_slice
        else:
            slices['pheno'] = torch.arange(0, batch[-1] + 2, dtype=torch.float)

    if data.behav is not None:
        if data.behav.size(0) == batch.size(0):
            slices['behav'] = node_slice
        else:
            slices['behav'] = torch.arange(0, batch[-1] + 2, dtype=torch.float)


   # print(torch.Tensor.size(slices['pheno']))
...
def read_data(data_dir):
    ...
    for j in range(len(res)):
        ...
        phenotype_list.append(res[j][5])
        behavior_list.append(res[j][6])

    print(y_list)
    #print(', '.join("%s: %s" % item for item in res.items()))
    ...
    phenotype_arr = np.stack(phenotype_list)
    behavior_arr = np.stack(behavior_list)
    ...
    phenotype_torch = torch.from_numpy(phenotype_arr).float()
    behavior_torch = torch.from_numpy(behavior_arr).float()
    ...
    data = Data(x=att_torch, edge_index=edge_index_torch, y=y_torch, edge_attr=edge_att_torch, pos = pseudo_torch, pheno = phenotype_torch, behav = behavior_torch)
...
def read_sigle_data(data_dir,filename,use_gdc =False):
    ...
    phenotype_torch = torch.from_numpy(np.array(phenotype)).float()
    behavior_torch = torch.from_numpy(np.array(behavior))

    data = Data(x=att_torch, edge_index=edge_index.long(), y=y_torch, edge_attr=edge_att, pheno=phenotype_torch, behav=behavior_torch)
    ...
    if use_gdc:
        ...
        return edge_att.data.numpy(),edge_index.data.numpy(),att,label,num_nodes,phenotype,behavior

if __name__ == "__main__":
    #data_dir = '/home/alex/hsm/GNN/BrainGNN_Pytorch/data/APOE2/full_connect/sex_vertex_32/raw'
    print('reading_data')
    #filename = '17.h5'
